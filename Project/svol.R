...
sp <- read.csv("~/GitHub/SDS383D-course-work/Project/SP1.csv")
sp=sp[c(1,2,3,4)]
...
n=length(sp$S.P.500)
y4=(sp$S.P.500[2:n]-sp$S.P.500[1:(n-1)])/sp$S.P.500[1:(n-1)]
plot(y4,type='l')
ar2=ar.ols(x=y4,order.max=1)

sample=sampler(y4,1,1,0,10,0,10,2000)
...
plot(log(sample$h_sample[100,]),type='l',ylab="log(h)",main="evolution")
